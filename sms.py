...
def add_data():
    if idEntry.get()==''or nameEntry.get()==''or emailEntry.get()==''or addressEntry.get()==''or dobEntry.get()=='':
...
def connect_database():
    def connect():
        ...
        try:
            ...
            return
        try:
          query='create database schoolmanagementsysteam'
          mycursor.execute(query)
          query='use schoolmanagementsysteam'
          mycursor.execute(query)
    ...
    hostnameLabel=Label(connectWindow,text='Host Name',font=('areal',20,'bold'))
    ...
    usernameLabel = Label(connectWindow, text='User Name', font=('areal', 20, 'bold'))
    usernameLabel.grid(row=1, column=0, padx=20)

    usernameEntry=Entry(connectWindow,font=('roman',15,'bold'),bd=2)
    usernameEntry.grid(row=1,column=1,padx=40,pady=20)

    passwordLabel = Label(connectWindow, text='Password', font=('areal', 20, 'bold'))
    passwordLabel.grid(row=2, column=0, padx=20)

    passwordEntry = Entry(connectWindow, font=('password', 15, 'bold'), bd=2)
...
root.resizable(0, 0)
root.title('school Manegement System')
...
deletestudentButton = ttk.Button(leftFrame, text='Delete Student', width=25, state=DISABLED,command=delet_student)
...
scrollBarX.config(command=studentTable.xview_scroll)
scrollBarY.config(command=studentTable.yview_scroll)
